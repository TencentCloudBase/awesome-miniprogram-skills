#!/bin/bash
# 批量执行 cli agent render（每个命令启动独立 IDE 实例）
...
# 已完成的跳过
SKIP="searchStores,getOrderStatus,searchDestinations,createPayment,estimateTrip,getStoreQueueStatus"
...
echo "=== 开始批量 render（共 $TOTAL 个API，每个独立 IDE 实例）==="
date
echo ""
...
for entry in "${APIS[@]}"; do
  ...
  "$CLI" agent render \
    ...
    --args="$args" \
    --port=0 \
    --timeout=120000 > "$OUTPUT/tmp/${api}_stdout.json" 2>/dev/null
  
  EC=$?
  if [ $EC -ne 0 ] && [ ! -s "$OUTPUT/tmp/${api}_stdout.json" ]; then
    echo "❌ CLI=$EC (无输出)"
    FAIL=$((FAIL + 1))
    continue
  fi
