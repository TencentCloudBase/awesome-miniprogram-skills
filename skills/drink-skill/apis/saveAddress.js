...
const { setAddress, getPendingOrder, findOrder, saveOrder, clearPendingOrder, isPreviewMode } = require('../utils/storage.js')
...
async function saveAddress({ name, phone, detail } = {}) {
  try {
    ...
    const { result } = await wx.cloud.callFunction({
      ...
      data: { action: 'saveAddress', name: address.name, phone: address.phone, detail: address.detail }
    })
    if (!result || result.code !== 0) {
      return errorResult(result?.message || '保存地址失败')
    }
